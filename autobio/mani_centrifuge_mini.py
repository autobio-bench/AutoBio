...
class CentrifugeMiniManipulate(Task):
    default_scene = SCENE_ROOT / "mani_centrifuge_mini.xml"
    default_task = "centrifuge_mini_close_lid"
...
class CentrifugeMiniManipulateExpert(CentrifugeMiniManipulate, Expert):
    ...
    def execute(self):
        ...
        if self.task == 'centrifuge_mini_close_lid':
            ...
            self.move_to(end_pose, num_steps=2)
            ...
            lock_pose_pre = self.instrument.get_eef_pose(self.data, loc='lid', mode='lock')
            self.gripper_control(255)
