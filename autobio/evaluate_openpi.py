...
def evaluate_openpi(openpi_root: Path, save_dir: Path, host: str, port: int, num_episodes: int, image_history: int, num_workers: int, render_device_id: str, dry_run: bool):
    checkpoint_root = openpi_root / "checkpoints"

    for task in checkpoint_root.iterdir():
        if not task.is_dir():
            continue
        if task.name == "thermal_cycler_combined":
            # thermal_cycler_combined is separately evaluated
            continue
        task_name = task.name
        if task_name.endswith("-lora"):
            task_name = task_name[:-5]

        task_seed = random.randint(0, 2**32 - 1)

        for experiment in task.iterdir():
            if not experiment.is_dir():
                continue
            experiment_name = experiment.name
            checkpoint_dir = experiment / "29999"
            if not checkpoint_dir.is_dir():
                raise RuntimeError(f"Checkpoint directory {checkpoint_dir} does not exist.")
            ...
            switch_args = ["--policy.config", task_name, "--policy.dir", str(checkpoint_dir.relative_to(openpi_root))]
            evaluate_args = [
            ...
            print(f"Would switch policy to {task_name} at {checkpoint_dir}")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Evaluate OpenPI checkpoints using the WebSocket client.")
