...
class Centrifuge5910Manipulate(Task):
    default_scene = SCENE_ROOT / "mani_centrifuge_5910.xml"
    default_task = "centrifuge_5910_lid_close"
    ...
    def reset(self, seed: int | None = None):
        ...
        # match task: 
        #     case "close":  
        #         prefix = 'close the lid of the centrifuge eppendorf 5910' 
        #     case "open":  
        #         prefix = 'open the lid of the centrifuge eppendorf 5910' 
    ...
    def check(self):
        # Add logic to check if the task is completed
        return True
...
class Centrifuge5910ManipulateExpert(Centrifuge5910Manipulate, Expert):
    ...
    def wait(self, wait_time, info):        
        dt = self.model.opt.timestep
        wait_steps = int(wait_time / dt)
    ...
    def move_to(self, pose: Pose, num_steps: int=2):
        ...
        if not path:
            raise ValueError("Path interpolation failed. Please check the input poses or interpolation logic.")
    ...
    def execute(self):
        ...
        if self.task == 'centrifuge_5910_lid_close':
