...
import openpi.policies.autobio_policy as autobio_policy
...
class DataConfig:
    ...
    repo_id: str | tuple[str] | None = None
    ...
    # If not None, will use the first `num_episodes` episodes from the dataset.
...
@dataclasses.dataclass(frozen=True)
class LeRoboAutoBioDataConfig(DataConfigFactory):
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        data_transforms = _transforms.Group(
            inputs=[autobio_policy.AutoBioInputs(action_dim=model_config.action_dim, model_type=model_config.model_type)],
            outputs=[autobio_policy.AutoBioOutputs(action_dim=self.real_action_dim)],
        )

        if self.real_action_dim == 7:
            delta_action_mask = _transforms.make_bool_mask(6, -1)
        elif self.real_action_dim == 14:
            delta_action_mask = _transforms.make_bool_mask(6, -1, 6, -1)
        else:
            raise ValueError(f"Unsupported action dimension: {self.real_action_dim}")
        data_transforms = data_transforms.push(
            inputs=[_transforms.DeltaActions(delta_action_mask)],
            outputs=[_transforms.AbsoluteActions(delta_action_mask)],
        )
...
def add_config(*configs: TrainConfig):
    for config in configs:
        ...
        lora_config = dataclasses.replace(
            config,
...
add_config(
    ...
    TrainConfig(
        ...
        model=pi0.Pi0Config(),
        data=LeRoboAutoBioDataConfig(
            repo_id="data/thermal_cycler_open",
            base_config=DataConfig(local_files_only=True, prompt_from_task=True),
        ),
        weight_loader=weight_loaders.CheckpointWeightLoader("s3://openpi-assets/checkpoints/pi0_base/params"),
        num_train_steps=30_000,
    ...
    TrainConfig(
        ...
        model=pi0.Pi0Config(),
        data=LeRoboAutoBioDataConfig(
            repo_id="data/thermal_cycler_close",
            base_config=DataConfig(local_files_only=True, prompt_from_task=True),
        ),
        weight_loader=weight_loaders.CheckpointWeightLoader("s3://openpi-assets/checkpoints/pi0_base/params"),
        num_train_steps=30_000,
    ...
    TrainConfig(
        ...
        model=pi0.Pi0Config(),
        data=LeRoboAutoBioDataConfig(
            repo_id="data/insert_centrifuge_5430",
            base_config=DataConfig(local_files_only=True, prompt_from_task=True),
        ),
        weight_loader=weight_loaders.CheckpointWeightLoader("s3://openpi-assets/checkpoints/pi0_base/params"),
        num_train_steps=30_000,
    ...
    TrainConfig(
        name="thermal_cycler_combined",
        model=pi0.Pi0Config(),
        data=LeRoboAutoBioDataConfig(
            repo_id=["data/thermal_cycler_open", "data/thermal_cycler_close"],
            base_config=DataConfig(local_files_only=True, prompt_from_task=True),
            assets=AssetsConfig(asset_id="thermal_cycler_combined"),
        ),
        weight_loader=weight_loaders.CheckpointWeightLoader("s3://openpi-assets/checkpoints/pi0_base/params"),
        num_train_steps=30_000,
    ),
