...
def create_dataset(data_config: _config.DataConfig, model_config: _model.BaseModelConfig) -> Dataset:
    ...
    if not isinstance(repo_id, str):
        assert isinstance(repo_id, Sequence) and len(repo_id) > 0
        assert episodes is None, "Multiple datasets cannot be used with num_episodes."
        all_fps, all_tasks = [], []
        for r in repo_id:
            dataset_meta = lerobot_dataset.LeRobotDatasetMetadata(r, local_files_only=data_config.local_files_only)
