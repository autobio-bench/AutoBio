...
class RepackTransform(DataTransformFn):
    ...
    def __call__(self, data: DataDict) -> DataDict:
        ...
        return jax.tree.map(lambda k: flat_item.get(k), self.structure)
    ...
    def __call__(self, data: DataDict) -> DataDict:
        ...
        if isinstance(self.tasks, list):
            if "dataset_index" not in data:
                raise ValueError('Cannot extract prompt without "dataset_index"')
            dataset_index = int(data["dataset_index"])
            assert 0 <= dataset_index < len(self.tasks)
            if (prompt := self.tasks[dataset_index].get(task_index)) is None:
                raise ValueError(f"{task_index=} not found in task mapping: {self.tasks[dataset_index]}")
        elif (prompt := self.tasks.get(task_index)) is None:
