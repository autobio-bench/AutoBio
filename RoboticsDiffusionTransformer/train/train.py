...
def train(args, logger):
    ...
    train_dataset = VLAConsumerDataset(
        ...
        use_lerobot=args.load_from_lerobot,
    ...
    if accelerator.is_main_process:
        train_dataset.lerobot_dataset.save_normalization(
            os.path.join(args.output_dir, "normalization.json"),
        )
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("rdt", config=vars(args), init_kwargs={"wandb": {"name": os.environ.get("EXP_NAME")}})
    ...
    accumulate_logs = defaultdict(list)
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for batch in train_dataloader:
            ...
            progress_bar.set_postfix(loss=loss, lr=lr)
            # logs.update(loss_for_log)
            # logger.info(logs)
            accumulate_logs["loss"].append(loss)
            accumulate_logs["lr"].append(lr)
            if global_step % 100 == 0:
                mean_loss = sum(accumulate_logs["loss"]) / len(accumulate_logs["loss"])
                mean_lr = sum(accumulate_logs["lr"]) / len(accumulate_logs["lr"])
                logs = { "loss": mean_loss, "lr": mean_lr }
                accelerator.log(logs, step=global_step)
                accumulate_logs.clear()
